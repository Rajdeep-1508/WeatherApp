...
export class WeatherService {
  ...
  getSummaryImage(summary:string):string{
    ...
    if(String(summary).includes("Partly Cloudy")||String(summary).includes("Mostly Cloudy"))return baseAddress + cloudySunny;
    else if(String(summary).includes("Mostly Cloudy"))return baseAddress + rainSunny;
    else if(String(summary).includes("Foggy"))return baseAddress + windy;
    else if(String(summary).includes("Few Showers")||String(summary).includes("Showers")||String(summary).includes("Rain")||String(summary).includes("Scattered Thunderstorms")||String(summary).includes("Thunderstorms"))return baseAddress + rainy;
    else if(String(summary).includes("Sunny")||String(summary).includes("Clear")||String(summary).includes("Mostly Clear")||String(summary).includes("Mostly Sunny"))return baseAddress + sunny;
  ...
  fillTodayData(){
    ...
    while(todayCount < 7){
      ...
      this.todayData[todayCount].summaryImage = this.getSummaryImage(this.weatherDetails['v3-wx-forecast-hourly-10day'].wxPhraseLong[todayCount]);
      todayCount++;
    }
  }
